		...
		public function view($start=0)
			{
				...
				$data['stat_data'] = $this->mod_publisher->get_payment_statistics_for_publisher($search_arr,$start,$limit);
			
				//print_r($data['stat_data']);exit;
				
				/*-------------------------------------------------------------
					Embed current page content into template layout
				-------------------------------------------------------------*/
				$data['page_content']		=		 $this->load->view("statistics/publisher/payments",$data,true);
		...
		else
		{
			...
			public function view_more_details(){
				...
				if($acc_id != false){
					...
					$data['publisher_list'] = $this->mod_publisher->get_publisher_monthly_revenue($acc_id);
					
					$this->load->view('statistics/publisher/publisher_revenue',$data);
				}
				else
				{
					echo '';
			...
			public function export_payment_history()
			{
				...
				$search_arr	=	 $this->session->userdata('statistics_search_arr');
				$data['publisher_list'] = $this->mod_websites->get_site_list("all");
				$data['stat_data'] = $this->mod_publisher->get_payment_statistics_for_publisher($search_arr);
				$this->mod_stat_excel->export_pub_payment_history_excel($data);
