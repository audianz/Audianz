...
class Mod_optimisation extends CI_Model
{
	...
	{
		$sql="REPLACE INTO `Campaign_Optimize_Mix` (`Mix_id`,`Network_type`,`Network`,`Mix_percent`,`cpm_rate`)
			 VALUES ('".$data[mix_id]."','".$data[type]."','".$data[network]."','".$data[mix_percent]."','".$data[cpm_rate]."') ";
		$query=$this->db->query($sql);
	...
	{
		$sql="REPLACE INTO `Campaign_Optimize_Mix` (`Network_type`,`Network`,`Mix_percent`,`cpm_rate`)
			 VALUES ('".$data[type]."','".$data[network]."','".$data[mix_percent]."','".$data[cpm_rate]."') ";
		$query=$this->db->query($sql);
		if($this->db->affected_rows()>0)
		{
			return true;
