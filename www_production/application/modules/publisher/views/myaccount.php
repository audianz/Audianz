...
<?php echo validation_errors();?>
        ...
        <form id="myaccount"  name="myaccount"  method="post" action="<?php echo site_url('publisher/myaccount/myaccount_settings_update');?>" enctype="multipart/form-data">
        	...
        	<div class="form_default">
				...
				<label for="website">Website</label>
					...
					readonly="readonly"
					disabled="disabled"
					class="validate[required] sf" value="<?php echo $row_record->websitename;?>" readonly='true' id="website"/>
				</p>				
				<p>
				<label for="name"><?php echo $this->lang->line('label_admin_myaccount_username');?><span style="color:red;">*</span></label>
					...
					value="<?php echo form_text((set_value('username') != '')?set_value('username'):$row_record->username);?>" readonly='true' id="email"/>
				</p>				
	 ...
	 <div class="notification msginfo"  style="width:33%;margin-left:170px;padding:5px;background:#C7D6EC;font-size:10px;margin-top:-7px;" ><?php echo $this->lang->line('lang_site_settings_image_upload_notice');?>
	</div
	><p>
...
if($row_record->avatar != ''): ?>
      <img  style="margin-left:10px;" src="<?php echo base_url().$this->config->item('user_img_view').$row_record->avatar; ?>" alt="<?php echo $row_record->username; ?>" title="<?php echo $row_record->username; ?>" />
<a href="javascript:menu_confirm()"><span style="margin-left:10px;"> <?php echo "[-] ".$this->lang->line('label_delete_profile'); ?></span></a>
      <?php endif; 
 ?> </p>

	<?php
		$qry	=$this->db->query("SELECT oac_category_id FROM ox_affiliates WHERE account_id=".$row_record->accountid);
		$rs		=$qry->result();
		foreach($rs as $rec) { $oac_category_id	=$rec->oac_category_id; }
	
		$sel_category	=explode(",", $oac_category_id);
		$options	=array();
	...
	<p>
	   ...
	   <?php echo form_multiselect('category[]', $options, @$sel_category, "class='sf'"); // "class='validate[required] sf' alt='".$this->lang->line('label_select_campaign_category')."'" ?>
