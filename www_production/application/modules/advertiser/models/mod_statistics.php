...
class Mod_statistics extends CI_Model {    
		...
		{ 
			...
			$SQL = "SELECT date(h.date_time) as db_date,
						oxcl.clientid, 
			...
			$SQL = "SELECT date(oxu.date_time) as db_date,
						oxcl.clientid, 
			...
			$SQL .=" GROUP BY date(oxu.date_time),`viewer_id` ,  `creative_id` ,  `zone_id` ";
			 ...
			 if($query->num_rows>0)
			 {		
				// Here $query->result_array() is not working
			 	$stat_summary = $query;
		 
			foreach($stat_summary as $unique_data){
			//while(!$query->EOF)
			//	{	$unique_data=$query->next_row('array');	 	
		...
		$workSheet->getStyle('B9:I9')->applyFromArray($tabletitleArray);
		...
		$workSheet->SetCellValue('B9',$this->lang->line('label_reports_poiname'));
		$workSheet->SetCellValue('C9', $this->lang->line('label_reports_city'));
		$workSheet->SetCellValue('D9',$this->lang->line('label_reports_imp'));
		$workSheet->SetCellValue('E9',$this->lang->line('label_reports_clk'));
		$workSheet->SetCellValue('F9', $this->lang->line('label_reports_landing'));
		$workSheet->SetCellValue('G9', $this->lang->line('label_reports_call'));
		$workSheet->SetCellValue('H9', $this->lang->line('label_reports_web'));
		$workSheet->SetCellValue('I9', $this->lang->line('label_reports_map'));
		...
		if(count($stat_list)>0)
		{
			...
			{
				$workSheet->getStyle('B'.$row.':I'.$row)->getFont()->setBold(true);

				$workSheet->setCellValueByColumnAndRow(1,$row,$adv['POI_NAME']);
				$workSheet->setCellValueByColumnAndRow(2,$row,$adv['CITY']);
				$workSheet->setCellValueByColumnAndRow(3,$row,$adv['IMP']);
				$workSheet->setCellValueByColumnAndRow(4,$row,$adv['CLK']);
				$workSheet->setCellValueByColumnAndRow(5,$row,$adv['LAND']);
				$workSheet->setCellValueByColumnAndRow(6,$row,$adv['CALL']);
				$workSheet->setCellValueByColumnAndRow(7,$row,$adv['WEB']);
				$workSheet->setCellValueByColumnAndRow(8,$row,$adv['MAP']);
			...
			$workSheet->setCellValueByColumnAndRow(3,$row,$final_tot['IMP']);
			$workSheet->setCellValueByColumnAndRow(4,$row,$final_tot['CLK']);
			$workSheet->setCellValueByColumnAndRow(5,$row,$final_tot['LAND']);
			$workSheet->setCellValueByColumnAndRow(6,$row,$final_tot['CALL']);
			$workSheet->setCellValueByColumnAndRow(7,$row,$final_tot['WEB']);
			$workSheet->setCellValueByColumnAndRow(8,$row,$final_tot['MAP']);
