...
class Mod_statistics extends CI_Model {    
		...
		function get_perfomance_report_for_advertiser_datewise($search_array){ 
   		...
   		function get_store_report_for_advertiser_datewise($search_array)
   		{
   			
   			
   			$result = array();
   			$total=array("IMP"=>0,"CLK"=>0,"LAND"=>0,"CALL"=>0,"WEB"=>0,"MAP"=>0);
   			$SQL = "SELECT 
   					date(oxr.date) AS 'db_date',
   					oxr.clientid AS 'CLIENT_ID',
   					st.poi_name AS 'POI_NAME',
   					st.address1 AS ADDR1,
   					st.address2 AS ADDR2,
   					st.city AS CITY,
					ifnull(sum(oxr.click_to_call),0) AS 'CALL',
					ifnull(sum(oxr.click_to_web),0) AS 'WEB',
					ifnull(sum(oxr.click_to_map),0) AS 'MAP',
					ifnull(sum(oxr.landing),0) AS 'LAND'
					FROM `oxm_report` AS oxr
					JOIN storefrontdata AS st ON oxr.poi_id=st.id 
					";
   				
   			if(isset($search_array['sel_advertiser_id'])  AND $search_array['sel_advertiser_id'] != '')
   			{
   				$SQL .=" WHERE oxr.clientid='".$search_array['sel_advertiser_id']."'";
   			}
   			
   			if(count($search_array) > 0  AND isset($search_array['from_date']) AND isset($search_array['to_date']) AND $search_array['from_date'] != '' AND $search_array['to_date'] != '')
   			{
   			
   				$SQL .=  " AND date(oxr.date) BETWEEN '".$search_array['from_date']."' AND '".$search_array['to_date']."'";
   			}
   			   				
   			
   			
   			$SQL .=" GROUP BY date(oxr.date)";
   			...
   			if($query->num_rows>0)
   			{
   				...
   				foreach($stat_summary as $data)
   				{
   					$poi	=	$data['POI_NAME'].", ".$data['ADDR1']." ".$data['ADDR2'];
   					$result[$data['db_date']]	=	array(
   							"date"=>$data['db_date'],
   							"IMP"=>0,
   							"CLK"=>0,
   			...
   			$SQL_STORE	=	"SELECT date( oxr.date ) AS 'db_date', oxr.clientid AS 'CLIENT_ID', 
   							st.poi_name AS 'POI_NAME',
   					        st.address1 AS ADDR1,
   							st.address2 AS ADDR2,
   							st.city AS CITY, 
   							ifnull( sum( oxr.impressions ) , 0 ) AS 'IMP',
   					    	ifnull( sum( oxr.clicks ) , 0 ) AS 'CLK'
							FROM `oxm_store_report` AS oxr
							JOIN storefrontdata AS st ON oxr.poi_id = st.id
							";
   			
   			if(isset($search_array['sel_advertiser_id'])  AND $search_array['sel_advertiser_id'] != '')
   			{
   				$SQL_STORE .=" WHERE oxr.clientid='".$search_array['sel_advertiser_id']."'";
   			}
   			
   			if(count($search_array) > 0  AND isset($search_array['from_date']) AND isset($search_array['to_date']) AND $search_array['from_date'] != '' AND $search_array['to_date'] != '')
   			{
   			
   				$SQL_STORE .=  " AND date(oxr.date) BETWEEN '".$search_array['from_date']."' AND '".$search_array['to_date']."'";
   			}
   				
   			$SQL_STORE .=" GROUP BY date(oxr.date)";
   			
   			$query = $this->db->query($SQL_STORE);
   			...
   			if($query->num_rows>0)
   			{
   				...
   				foreach($store_data as $data_imp)
   				{
   					...
   					{
   						$poi	=	$data_imp['POI_NAME'].", ".$data_imp['ADDR1']." ".$data_imp['ADDR2'];
   						$tot_imp									=	$data_imp['IMP'];
   						$result[$data_imp['db_date']]['IMP']		=	$tot_imp;
   						$result[$data_imp['db_date']]['date']		=	$data_imp['db_date'];
   						$result[$data_imp['db_date']]['CLK']		=	$data_imp['CLK'];
   						$result[$data_imp['db_date']]['POI_NAME']	=	$poi;
   						$result[$data_imp['db_date']]['CITY']		=	$data_imp['CITY'];
   						
   							
   					}
   					if(isset($temp[$data_imp['db_date']]['CLK']))
   					{
   						$tot_clk									=	$data_imp['CLK'] + $temp[$data_imp['db_date']]['CLK'];
   						$result[$data_imp['db_date']]['CLK']		=	$tot_clk;
   					}
   					else
   					{
   						$poi	=	$data_imp['POI_NAME'].", ".$data_imp['ADDR1']." ".$data_imp['ADDR2'];
   						$tot_clk									=	$data_imp['CLK'];
   						$result[$data_imp['db_date']]['CLK']		=	$tot_clk;
   						$result[$data_imp['db_date']]['IMP']		=	$data_imp['IMP'];
   						$result[$data_imp['db_date']]['date']		=	$data_imp['db_date'];
   					...
   					if(!isset($temp[$data_imp['db_date']]['LAND']))
   					{
   						$result[$data_imp['db_date']]['LAND']		=	0;	
   					}
   					if(!isset($temp[$data_imp['db_date']]['CALL']))
   					{
   						$result[$data_imp['db_date']]['CALL']		=	0;
   					}
   					if(!isset($temp[$data_imp['db_date']]['WEB']))
   					{
   						$result[$data_imp['db_date']]['WEB']		=	0;
   					}
   					if(!isset($temp[$data_imp['db_date']]['MAP']))
   					{
   						$result[$data_imp['db_date']]['MAP']		=	0;
   					}
   			...
   			foreach ($result as $tot_val)
   			{
   				$total['IMP']	=	$total['IMP']+$tot_val['IMP'];
   				$total['CLK']	=	$total['CLK']+$tot_val['CLK'];
   				$total['CALL']	=	$total['CALL']+$tot_val['CALL'];
   				$total['WEB']	=	$total['WEB']+$tot_val['WEB'];
   				$total['MAP']	=	$total['MAP']+$tot_val['MAP'];
   				$total['LAND']	=	$total['LAND']+$tot_val['LAND'];
   			}
   			
   			
   			$out=array("stat_list"=>$result,"tot_val"=>$total);
   			
   			return $out;
   		}

	    function export_storefront_excel($data='')
	    {
	    
	    	$stat_data		=	$data['stat_data'];
	    	$stat_list		=	$stat_data['stat_list'];
	    	$final_tot		=	$stat_data['tot_val'];
		...
		$workSheet->getStyle('B9:J9')->applyFromArray($tabletitleArray);

			
		//set default heading and location
		$objPHPExcel->setActiveSheetIndex(0);
			
		$workSheet->SetCellValue('B3', $this->lang->line('label_reports_adv_store'));
		$workSheet->mergeCells('B6:C7');
	

		$workSheet->SetCellValue('B9',$this->lang->line('label_reports_date'));
		$workSheet->SetCellValue('C9',$this->lang->line('label_reports_poiname'));
		$workSheet->SetCellValue('D9', $this->lang->line('label_reports_city'));
		$workSheet->SetCellValue('E9',$this->lang->line('label_reports_imp'));
		$workSheet->SetCellValue('F9',$this->lang->line('label_reports_clk'));
		$workSheet->SetCellValue('G9', $this->lang->line('label_reports_landing'));
		...
		if(count($stat_list)>0)
		{
			...
			foreach($stat_list as $adv)
			{
				$workSheet->getStyle('B'.$row.':J'.$row)->getFont()->setBold(true);
				$workSheet->setCellValueByColumnAndRow(1,$row,$adv['date']);
				$workSheet->setCellValueByColumnAndRow(2,$row,$adv['POI_NAME']);
				$workSheet->setCellValueByColumnAndRow(3,$row,$adv['CITY']);
				$workSheet->setCellValueByColumnAndRow(4,$row,$adv['IMP']);
				$workSheet->setCellValueByColumnAndRow(5,$row,$adv['CLK']);
				$workSheet->setCellValueByColumnAndRow(6,$row,$adv['LAND']);
			...
			$workSheet->setCellValueByColumnAndRow(4,$row,$final_tot['IMP']);
			$workSheet->setCellValueByColumnAndRow(5,$row,$final_tot['CLK']);
			$workSheet->setCellValueByColumnAndRow(6,$row,$final_tot['LAND']);
		...
		else
		{
			...
			$camp_title_style = array();
			$camp_style	=	array();
			$content="There are no statistics for this Advertiser's storefront ".$fromdate."--".$todate;
