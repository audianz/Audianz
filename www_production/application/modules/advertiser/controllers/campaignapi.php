...
class Campaignapi extends CI_Controller
{
		...
		if(isset($attributes['api']))
		{
			switch($attributes['api'])
			{
                ...
                case 'update_campaign_request':
                    $this->handleUpdateCampaignDetail($attributes);
                    break;
				default:
					//echo "default";
					break;
		...
		if($resultset!=false)
		{
		     $usd_rate = $resultset->usd_rate;
                ...
                foreach($camplist as $cmp)
                {
                    ...
                    $getAction =$this->mod_campaigns->check_targeting_campaign($cmp->campaignid);
					$result[$count]['action'] = $getAction->landing_page;
	...
	/**
	 * Author: Soumya
	 * This function is used to update campaign details in case of compliance status.
	 */
	 
	 public function handleUpdateCampaignDetail($attributes)
	 {
		$this->logger->logdebug('Campaignapi::handleUpdateCampaignDetail()  entered');	
		if(!empty($attributes['client_id']) and !empty($attributes['campaign_id']) and !empty($attributes['promote_msg']) and !empty($attributes['click_to_action']) )
		{$advertiser = $attributes['client_id'];
			...
			$msg = $attributes['promote_msg'];		
			$click_action= $attributes['click_to_action'];	
			...
			$update_res = $this->mod_campaigns->update_campaign_details($camp_id,$edit_campaign);
			if($upadte_res!=FALSE)
			{
				/** Set click to action   **/
		...
		{
			$this->logger->logerror('Campaignapi::handleUpdateCampaignDetail()  required parameters are missing');
			$response = array(
					STATUS=>ERR_STATUS,
					ERR_CODE_KEY =>PARAM_REQ,
					ERR_STR =>PARAM_REQ_STR
