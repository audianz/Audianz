...
class Suggestions extends CI_Controller {
    ...
    {
		parent::__construct();
		$this->sessid	=$this->session->userdata('session_advertiser_id');
	...
	{
		redirect('advertiser/suggestions/suggestion_list');
	}
    ...
    function suggestion_list($type=0, $start =0)
	{
		...
		$config['base_url'] = site_url("advertiser/suggestions/suggestion_list/".$type.'/');
		...
		$where				 =array('oxs.suggestion_reciever' =>$this->sessid); //'oxs.suggestion_status' =>$type, 
 	    $data['rs']		     =$this->mod_suggestions->get_read_advertiser_listing($where, $limit, $start);
    ...
    function view($id=false)
	{
		...
		else
		{
			...
			$where				 =array('oxs.suggestion_id' =>$id , 'oxs.suggestion_reciever' =>$this->sessid); //,'oxs.suggestion_status' =>0
 	    	$data['rs']		     =$this->mod_suggestions->get_read_advertiser_listing($where);
