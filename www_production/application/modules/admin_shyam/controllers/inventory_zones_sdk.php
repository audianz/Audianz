<?php
class inventory_zones_sdk extends CI_Controller {  
    ...
    {  
		...
		//loc: inventory/models/mod_banners
    }
	...
	{ 
		...
		$config['base_url'] 			= 	site_url("admin/inventory_zones_sdk/listing/".$affiliateid."/");
		...
		$list_data = $this->mod_zones->list_zones($where,$start,$limit);
		...
		$data['page_title'] 	=	$this->lang->line('label_inventory_mobile_page_title');
		
		/*-------------------------------------------------------------
		 	Embed current page content into template layout
		 -------------------------------------------------------------*/
		 
		$data['page_content']	=	$this->load->view("zones/inventory_zones_sdk",$data,true);
	...
	public function insert($affiliateid=0) 
	{
		...
		redirect("admin/inventory_zones_sdk/listing/".$affiliateid);
	...
	/* admin/inventory_zones_sdk/Edit zones controller */
	
	public function edit_zones_sdk($affiliateid=0,$zoneid=0) 
	{ 
		/*-----------------------------------------------------------
		 			Breadcrumb Setup Start
		 ------------------------------------------------------------*/
		...
		$data['page_content']	= $this->load->view("zones/edit_zones_sdk",$data,true);
		$this->load->view('page_layout',$data);
		
	}

	/* admin/inventory_zones_sdk/Update zones controller */
	...
	{ 
			...
			else
			{
				...
				$this->mod_zones->update_zones($zonename, $pricing, $zoneid, $date,$model_value);
	...
	/* admin/inventory_zones_sdk/Delete zones controller */
	...
	public function process($task){
		...
		if($task=="linkedbycampaigns"){
			...
			foreach($sel_campaign_list as $val)
			  ...
			  redirect("admin/inventory_zones_sdk/linkedbycampaigns/".$sel_zone_id.'/'.$affiliate_id);
		}
		else{
			...
			foreach($sel_banner_list as $val)
			  ...
			  redirect("admin/inventory_zones_sdk/linked_by_banners/".$sel_zone_id.'/'.$affiliate_id);
		}
		else{
		...
		{
			redirect("admin/inventory_zones_sdk");
		}
	...
	{
	$data = file_get_contents("assets/mobilesdk/mJAXMobileAppAdsAndroid.zip"); // Read the file's contents
	$name = 'mJAXMobileAppAdsAndroid.zip';
...
/* Location: ./modules/admin/controllers/inventory_zones_sdk.php */
