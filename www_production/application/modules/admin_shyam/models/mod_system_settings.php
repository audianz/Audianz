...
// get the country all the records
	...
	/***** Retreive  Network Carriers *******/

	function get_network_carriers($status="all",$limit=0,$offset=0)

	{
			...
			$this->db->from("ox_carrier_detail");
	...
	function check_carrier_name_exist($carriername, $country)
	{
		$this->db->select('*');
		$this->db->from('ox_carrier_detail as oxcd');
		$this->db->where('oxcd.carriername', $carriername);
		$this->db->where('oxcd.country', $country);
	...
	function check_start_ip_exist($start_ip)
	{
		$this->db->select('*');
		$this->db->from('ox_carrier_detail as oxcd');
		$this->db->where('oxcd.start_ip', $start_ip);
		$this->db->or_where('oxcd.end_ip', $start_ip);
	...
	function check_end_ip_exist($end_ip)
	{
		$this->db->select('*');
		$this->db->from('ox_carrier_detail as oxcd');
		$this->db->where('oxcd.end_ip', $end_ip);
		$this->db->or_where('oxcd.start_ip', $end_ip);
