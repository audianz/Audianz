...
class Mod_campaign extends CI_Model
{
	...
	function retrieve_campaign_count()
	{
		$query = $this->db->get('ox_campaigns');
	...
	function check_targeting_campaign($where_camp)
	{
		if($where_camp!=0)
		{
			$this->db->where('dtl.campaignid',$where_camp);
		}
		//$this->db->join('djx_campaign_limitation as dcl', 'dcl.campaignid=dtl.campaignid');
		$query = $this->db->get('djx_targeting_limitations as dtl' );
		//echo $this->db->last_query();exit;
		return $query->result();
	}
	...
	//Retreive comparison Options based on type
	function get_comp_options($type=FALSE)
	{
		if($type !=FALSE)
		{
			$comp_sql = "SELECT op.* FROM targ_delivery_type as dt,targ_operations as op WHERE op.status=1 AND FIND_IN_SET(op.id,dt.operation_id) AND dt.name='$type' ORDER BY CASE WHEN op.name like 'is%' THEN 0 ELSE 3 END, op.id ASC " ;
			...
			if($query->num_rows>0)
			{
				$value =array();
				$name= array();
				foreach($query->result() as $comp_op)
				{
					array_push($value,$comp_op->value);
					array_push($name,$comp_op->name);
				}
				$options = array_combine($value,$name);
