...
{ 
		var $table='ox_zones';
			...
			function get_edit_zones($zoneid)
			{
			...
			function update_zones($zonename, $pricing, $zoneid,$date,$revenue_type)
			{
			...
			function list_linked_campaign_data($zoneid=0, $limit=FALSE, $offset=0)
			{
				...
				$t = $this->get_records($select='affiliateid', "ox_zones", array("zoneid"=>$zoneid));
				
				$category = $this->db->select('oac_category_id')->get_where('ox_affiliates',array('affiliateid'=>$t->affiliateid))->row_array();
				...
				if(!empty($category['oac_category_id']))
				{
					...
					$this->db->where("oxb.adminstatus !=",1);
