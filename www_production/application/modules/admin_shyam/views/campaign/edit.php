...
?>
<form id="form_edit_campaign" action="<?php echo site_url('admin/inventory_campaigns/edit_campaign_process/'.$campaignidpro.'/'.$sel_advertiser); ?>" method="post">
<h1 class="pageTitle"><?php echo $this->lang->line('label_inventory_edit_campaign'); ?></h1>
		...
		if($this->session->userdata('camp_error') != ""): ?>
				<div class="notification msgerror"><a class="close"></a><p><?php echo $this->session->userdata('camp_error'); ?></p></div>
                <?php
				$this->session->unset_userdata('camp_error');
            endif;
            
            /*if($this->session->userdata('validation_error') != ""): ?>
				<div class="notification msgerror"><a class="close"></a><p><?php echo $this->session->userdata('validation_error'); ?></p></div>
                <?php
		...
		<input type="hidden" name="camp_link_count" id="camp_link_count" value="<?php echo $link_count; ?>" />
            ...
            <p>
                ...
                <select name="advertiser" id="advertiser" class="validate[required]" disabled="disabled">
                    ...
                    <?php if(count($advertiser)>0){
                            ...
                            {
                                if($campaign->clientid == $advertiser[$i]->clientid)
                                {
                            ?>
...
<input type="hidden" name="advertiser" id="advertiser" value="<?php echo $campaign->clientid; ?>" />
            </p>
            ...
            <p>
                ...
                <input type="radio" <?php echo form_text((((set_value('start_date') != '')?set_value('start_date'):$campaign->status_startdate)==0)?"checked":""); ?> onclick="startdate()"  name="start_date" id="start_date" value="0" /> <?php echo $this->lang->line('label_camp_start_immediately'); ?> &nbsp;&nbsp;&nbsp;&nbsp;
                    ...
                    <input id="campstart" name="campstart" type="text" class="validate[required]" alt="<?php echo $this->lang->line('label_alert_camp_start_date'); ?>"  value="<?php echo isset($campaign->activate_time)?date('m/d/Y H:i',strtotime($campaign->activate_time)):''; ?>"/>
            </p>
			...
			<p>
                <label for="email"><?php echo $this->lang->line('label_camp_end_date'); ?><span style="color:red;">*</span></label>
                 <input type="radio" <?php echo  form_text((((set_value('end_date') != '')?set_value('end_date'):$campaign->status_enddate)==0)?"checked":""); ?> onclick="enddate()" name="end_date" id="end_date" value="0" /> <?php echo $this->lang->line('label_camp_dont_expire'); ?> &nbsp;&nbsp;&nbsp;&nbsp;
                         ...
                         <input id="campend" name="campend" type="text" class="validate[required]" alt="<?php echo $this->lang->line('label_alert_camp_end_date'); ?>"  value="<?php echo isset($campaign->expire_time)?date('m/d/Y H:i',strtotime($campaign->expire_time)):''; ?>"/>
            </p>
			...
			<p id="rtbCamp">
                ...
                <input type="checkbox" name="rtb" id="rtb" onchange="selRTBcamp();" <?php if($campaign->rtb==1): ?> checked="checked" <?php endif; ?>/>&nbsp;&nbsp;Yes, I want to make this campign with Real-Time Bidding
			</p>
                <p>
		...
		{
		?>
<button type="button" style="margin-left:10px;" onclick="document.location.href='<?php echo site_url("admin/inventory_campaigns/listing_adv/all/".$sel_adv);?>'" ><?php echo $this->lang->line('label_cancel'); ?></button>
		<?php
		}
		?> 
            </p>
