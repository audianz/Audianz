...
class Paypal_Lib {
	...
	function Paypal_Lib()
	{
		...
		$this->paypal_url = 'https://sandbox.paypal.com/cgi-bin/webscr';
	...
	function button($value)
	{
		...
		$this->submit_btn = form_submit('pp_submit', $value,"class='button button_blue'");
	}
	...
	function paypal_auto_form() 
	{
		...
		echo '<img src='.base_url().'assets/images/loaders/loader7.gif alt="" />';
	...
	{	
		return $this->button('Click here if you\'re not automatically redirected...');
	}
	
	function get_paypal_form_content(){
	...
	{
		log_message('info', 'Validate IPN function Called Here...!');	
		// parse the paypal URL
		$url_parsed = parse_url($this->paypal_url);
		log_message('info', $url_parsed);
		...
		log_message('info',$post_string);
		log_message('info',$url_parsed['host']);
		...
		{ 
			...
			log_message('info', $url_parsed['path']);
			fputs($fp, "Host: $url_parsed[host]\r\n"); 
			log_message('info', $url_parsed['host']);
			fputs($fp, "Content-type: application/x-www-form-urlencoded"."\r\n"); 
			fputs($fp, "Content-length: ".strlen($post_string)."\r\n"); 
			log_message('info', strlen($post_string));
			fputs($fp, "Connection: close\r\n\r\n"); 
			fputs($fp, $post_string."\r\n\r\n"); 

			// loop through the response from the server and append to variable
			while(!feof($fp))
				$this->ipn_response .= fgets($fp, 1100);
 

			fclose($fp); // close connection
		}
		log_message('info', count($this->ipn_response));
		$ipnresp = explode('&',$this->ipn_response);
                //log_message('info', $ipnresp[0]);
                //log_message('info', $ipnresp[1]);
                //log_message('info', $ipnresp[2]);
		if (eregi("VERIFIED",$post_string))
		{
			// Valid IPN transaction.
			log_message('info', 'Success...IPN Validated!!!!');
		...
		else 
		{
			...
			log_message('info', $this->last_error);
			$this->log_ipn_results(false);	
			return false;
		}
	}
