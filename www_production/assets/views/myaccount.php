...
function checkValidateNew(field, rules, i, options){
 
           if(jQuery('#email_del').attr('checked')) {
    jQuery("#reports").show();
} else {
    jQuery("#reports").hide();
}
...
function textValidateNew(field, rules, i, options){
...
<?php echo validation_errors();?>
        ...
        <form id="myaccount"  name="myaccount"  method="post" action="<?php echo site_url('advertiser/myaccount/myaccount_settings_update');?>"  enctype="multipart/form-data">
        	...
        	<div class="form_default">
			 ...
			 alt="<?php echo $this->lang->line('label_admin_myaccount_username_validation');?>"
			  value="<?php echo form_text((set_value('username') != '')?set_value('username'):$row_record->username);?>" 		readonly='true'	
			 id="email" 
	 />
                    </p>				
	...
	<div class="notification msginfo"  style="width:26%;margin-left:170px;padding:5px;background:#C7D6EC;font-size:10px;margin-top:-7px;" >
...
if($row_record->avatar != ''): ?>
      <img  style="margin-left:10px; border:solid:0 0 #C7D6EC;" src="<?php echo base_url().$this->config->item('user_img_view').$row_record->avatar; ?>" alt="<?php echo $row_record->username; ?>" title="<?php echo $row_record->username; ?>" />
<a href="javascript:menu_confirm()"><span style="vertical-align:bottom;margin-left:10px;"> <?php echo "[-] ".$this->lang->line('label_delete_profile'); ?></span></a>
 ...
 ?> 
      ...
      <textarea name="address" 
				...
				id="address"><?php echo form_text((set_value('address') != '')?set_value('address'):$row_record->address);?></textarea>
    </p>                   
					<p>
    ...
    <p>
      ...
      <?php echo form_dropdown('country', $options_country,$sel_country,"class='validate[required] sf' alt='".$this->lang->line('lang_advertiser_enter_country')."'"); ?>
	         ...
	         id="state" 
	value="<?php echo form_text((set_value('mobileno') != '')?set_value('mobileno'):$row_record->mobileno);?>"/></p></fieldset>
				<br/>
				...
				<fieldset>
					...
					<p>
                    	<label for="name"><?php echo $this->lang->line("label_publisher_myaccount_paypal_id"); ?></label>
                              <input type="text"
	   		 name="paypalid"  
	          class="validate[funcCall[emailValidateNew]] sf" 
...
if($row_record->reportdeactivate=="t")
{
...
$checking="";
	...
	<br/>
				<fieldset>
                    <legend><?php echo $this->lang->line("label_advertiser_report"); ?></legend>
                    
					<p>
					<input type="checkbox" name="email_campaign" id="email_campaign" <?= $checking;?> value="t">&nbsp;&nbsp;<?php echo $this->lang->line("label_advertiser_email_activated/deactivated"); ?>
					</p>
					
					<p>
					<input type="checkbox" name="email_del" id="email_del" <?= $check;?> class="validate[required,funcCall[checkValidateNew]]" value="t">&nbsp;&nbsp;<?php echo $this->lang->line("label_advertiser_email_delivery_reports"); ?>
					</p>
					
					<p <?php echo $style; ?> id="reports">
                    	<?php echo $this->lang->line("label_advertiser_email_noofday_delivery_reports"); ?>
                        <input type="text"  name="camp_dely_rpt"  id="camp_dely_rpt"
     class="validate[required,funcCall[textValidateNew]] sf" 
                            alt="<?php echo "Please Enter the delivery reports days";?>"                    
value="<?php echo $row_record->reportinterval;?>"  />
                    </p>
	  ...
	  function menu_confirm()
		{
			...
			'<?php echo $this->lang->line("settings_site_delete_title"); ?>',function(r){
					if(r)
					{
					document.location.href	= '<?php echo site_url("advertiser/myaccount/myaccount_delete_avatar");?>';	
	}				});
					}
</script>
<!--fullpage-->
