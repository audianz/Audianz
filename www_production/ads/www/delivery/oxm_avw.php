...
$log->logInfo("Entered oxm_avw.php");
...
{
	if(is_null($_GET['lat']) || is_null($_GET['lon']))
	{
		return false;
	}
...
function device_os($limitation, $op)
{
	...
	if(!empty($details)) {
		...
		$result =  MAX_limitationsMatchArrayValue($details, $limitation, $op);
...
function _adRenderBuildClickUrl($aBanner, $zoneId=0, $source='', $ct0='', $logClick=true, $overrideDest=false)
{
	...
	{
		if($poiIndex[$i]['adId'] == $aBanner['bannerid'])
		{

			if(!empty($poiIndex[$i]['poiId']) && ($poiIndex[$i]['poiId'] != -1))
			{
				$currentPoiId = $poiIndex[$i]['poiId'];
				break;
			}
		}
	}
	...
	$clickUrl .= "&amp;".$conf['var']['lat']."=".$_GET['lat']."&amp;".$conf['var']['lon']."=".$_GET['lon']."&amp;".$conf['var']['POIKey']."=".$currentPoiId."&amp;type=2";
...
function _adSelectCheckCriteria($aAd, $aContext, $source, $richMedia)
{
	...
	if($location_limitation == 1)
	{
		...
		if($result == false)
		{
			$log->logDebug("oxm_avw ** adSelectCheckCriteria return false \n");
		}
		return $result;
	}
	else if($location_limitation == 0)
	{
		$log->logDebug("oxm_avw  oxm_avw location_limitation == 0 \n");
	}
	return true;
}
//added by neha
...
function  _adLocationMatches($adId, $clientId)
{
	...
	if(mysql_num_rows($_distanceQuery) > 0)
	{
		...
		*/		
		...
		$log->logDebug("oxm_avw poi id value is :".$tempPoiIndx['poiId']."and ad id is : ".$tempPoiIndx['adId']);
	...
	{
		$log->logDebug("oxm_avw adLocationMatches storefront row not found \n");
	}
	mysql_free_result($_distanceQuery);
	if($result == false)
	{
		$log->logDebug("oxm_avw ** adLocationMatches return false \n");
	}
	return $result;
}
...
//added by neha
function getNewIframe($iframeStr)
{


	$iframeTemp;
	$tempArrayOne = explode("iframe", $iframeStr);

	$tempArraySrc = explode(">",$tempArrayOne[1]);

	$iframeTemp = "<iframe".$tempArraySrc[0]."><\/iframe>";
	$tmp = str_replace("\\","",$iframeTemp);
	return $tmp;
}
...
if(!empty($output['bannerid'])){
	...
	for($i = 0; $i < count($poiIndex); $i++)
	{
		...
		if($poiIndex[$i]['adId'] == $output['bannerid'])
		{
	...
	$path="http://".$confa['webpath']['deliverySSL']."/oxm_ref.php?zoneid={$output['aRow']['zoneid']}&amp;refresh={$refresh}&amp;gender={$gender}&amp;age={$age}&amp;teleco={$teleco}&lat={$lat}&lon={$lon}&bnrid={$output['bannerid']}&poi=$poiVal";
	$echoVal =  MAX_javascriptToHTML("<iframe src='{$path}' frameborder='0' scrolling='no' width='{$output['width']}' height='{$output['height']}'>
	</iframe>", 'OX_'.substr(md5(uniqid('', 1)), 0, 8));
	$newIframe = getNewIframe($echoVal);
