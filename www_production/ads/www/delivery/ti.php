...
if (_viewersHostOkayToLog())
{
	...
	/* Retrieve  client id */
	$clientid=mysql_query("select * from ox_trackers where trackerid='$trackerId'") or die("culnot select1");
	$clientid1=mysql_fetch_array($clientid);
	$client_id=$clientid1['clientid'];
	$zid=$aConversion['zid'];
	$cid=$aConversion['cid'];

	$aff_id=mysql_query("select * from ox_zones where zoneid='$zid'") or die("culnot select2");
	$afff_id=mysql_fetch_array($aff_id);
	$affiliateid=$afff_id['affiliateid'];
	...
	/*Retrieve campaign id */
			$campaignid=mysql_query("select * from ox_banners where bannerid='$cid'") or die("culnot select3");
			$campaign=mysql_fetch_array($campaignid);
			$campaign_id=$campaign['campaignid'];
			$banner_con=mysql_query("select * from ox_campaigns WHERE campaignid=".$campaign_id."") or die("culnot select4");
			$ban_con=mysql_fetch_array($banner_con);
			$zonevalue=$ban_con['revenue'];
			$revenue_type=$ban_con['revenue_type'];
			$commission=$ban_con['commission'];
	
/*affiliates */		
$aff_com=mysql_query("select * from ox_affiliates where affiliateid='$affiliateid'") or die("culnot select5");
...
{
$account_value = mysql_query("Select * from oxm_accbalance where clientid='$client_id'") or die("culnot select6");
$budget_value=mysql_query("select * from oxm_budget where campaignid='$campaign_id'") or die("culnot select7");
...
$budget_val=mysql_fetch_array($budget_value);
			...
			$perclicks=mysql_query("select * from ox_campaigns WHERE campaignid=".$campaign_id."") or ("mysql_error()");
			$perclick1=mysql_fetch_array($perclicks);
			$perconv =$perclick1['revenue'];

			


			$acc_amount = $accamount -$perconv ;
			$bud_amount = $budamount -$perconv ;
			$date=date("Y-m-d");
			$monthvalue=substr($date,5,2);
			$amount=$amount+$perconv;
			$admin_commission=(($commission/100)*$perconv);
			$pub_commission=$perconv-$admin_commission;
			...
			{
			//mysql_query("update oxm_budget set budget='$dailybudget',currentdate='$date' where campaignid='$campaign_id'");
			}
			}
			 

			if(($accamount>=$zonevalue))
			{
			if($logconversions)
			{
			OX_Delivery_Common_hook('logConversion', array($trackerId,$serverRawIp,$aConversion));
				

		
			mysql_query("update oxm_accbalance set accbalance=".$acc_amount." where accbalance>0 AND  clientid=".$client_id."") or 					die("Error ---> 1".mysql_error()); 
				mysql_query("update oxm_budget set budget=".$bud_amount." where clientid=".$client_id." AND campaignid=".$campaign_id."") or die("Error ---> ".mysql_error());		

			$query=mysql_query("select * from oxm_report where clientid='$client_id' AND zoneid='$zid' AND bannerid='$cid' AND pricingmodel='$revenue_type'AND date='$date'") or die("culnot select8");
			...
			{
			$total=$tvalue+$perconv;
			$publisher_commission=$pubvalue+$pub_commission;
			mysql_query("update oxm_report set amount='$total',publisher_amount='$publisher_commission' where clientid='$client_id'AND zoneid='$zid' AND bannerid='$cid'AND pricingmodel='$revenue_type' AND date='$date'") or die("culnot select9");
			}
			else
			{
			mysql_query("insert into oxm_report(clientid,zoneid,campaignid,bannerid,amount,date,pricingmodel,publisherid,publisher_amount)values('$client_id','$zid','$campaign_id','$cid','$amount','$date','$revenue_type','$affiliateid','$pub_commission')") or die("Error 1".mysql_error());
			
			}

/*Credit table

			$credit=mysql_query("select * from oxm_credit where month='$month' AND year='$year' AND clientid='$client_id'") or die("culnot select10");
			$credit_amount=mysql_fetch_array($credit);
			$dbcreditamount=$credit_amount['amount'];
			$credit_last_date=$credit_amount['date'];
			if(mysql_num_rows($credit)>0)
			{
			$paypal=mysql_query("select Amount,date from oxm_paypal where month(date)='$month' AND year(date)='$year' AND clientid='$client_id' AND  date >'$credit_last_date'") or die("culnot select11");
			if(mysql_num_rows($paypal)>0)
			{
			$paypal_amount=mysql_fetch_array($paypal);
			$paypal_amount1=$paypal_amount['Amount'];
			$pal_date=mysql_query("select * from oxm_paypal where clientid='$client_id' AND month(date)='$month' AND year(date)='$year' ") or die("culnot select12");
			$papal_date=mysql_fetch_array($pal_date);
			 $paypal_date=$papal_date['date'];
			$creditvalue=($paypal_amount1+$dbcreditamount)-$perclick;
			mysql_query("update oxm_credit set amount='$creditvalue',date='$paypal_date' where month='$month' AND year='$year' AND clientid='$client_id' ")  or die("culnot select13");
			}
			else
			{
			$creditvalue=($paypal_amount1+$dbcreditamount)-$perclick;
			mysql_query("update oxm_credit set amount='$creditvalue' where month='$month' AND year='$year' AND clientid='$client_id' ")  or die("culnot select14");
			}
			}
			else
			{
			$paypal=mysql_query("select sum(Amount),date from oxm_paypal where month(date)='$month' AND year(date)='$year' AND clientid='$client_id'")  or die("culnot select15");
			$paypal_amount=mysql_fetch_array($paypal);
			$finalpaypal=$paypal_amount['sum(Amount)'];
			$paypaldate=$paypal_amount['date'];
			$totalamount=mysql_query("select sum(amount)from oxm_credit where clientid='$client_id'")  or die("culnot select16");
			$credit_amount=mysql_fetch_array($totalamount);
			$finalamount=$credit_amount['sum(amount)'];
			$final=($finalamount+$finalpaypal)-$perclick;
			mysql_query("insert into oxm_credit(clientid,amount,month,year,date)values('$client_id','$final','$month','$year','$paypaldate')")   or die("culnot select17");
			}



			$debit=mysql_query("select * from oxm_debit where month='$month' AND year='$year' AND publisherid='$affiliateid'")  or die("culnot select18");
			$debit_value=mysql_fetch_array($debit);
			if(mysql_num_rows($debit)>0)
			{

			$tot_amount=$debit_value['totalamount']+$pub_commission;
			mysql_query("update oxm_debit set totalamount='$tot_amount' where month='$month' AND year='$year'AND publisherid='$affiliateid'")  or die("culnot select19");
			}
			else
			{

			mysql_query("insert into oxm_debit(publisherid,totalamount,month,year)values('$affiliateid','$pub_commission','$month','$year')")  or die("culnot select20");	
			}
*/
			}
			}
...
$queryadvertiser=mysql_query("select * from ox_campaigns where campaignid=".$campaign_id."") or die("mysql_error()");
$norow=mysql_fetch_array($queryadvertiser);
$mailid=mysql_query("select * from ox_clients where clientid=".$client_id."") or die("mysql_error()");
...
mysql_query("UPDATE oxm_accbalance SET mail_status = '1' WHERE clientid =".$client_id)or die('error in Update');
...
mysql_query("UPDATE oxm_budget SET dailymail_status = '1' WHERE clientid =".$client_id)or die('error in Update');
...
{
		// UPDATE CONVERSIONS FOR CPM AND CPC Campaigns or Zones
			if($logconversions)
			{
				OX_Delivery_Common_hook('logConversion', array($trackerId,$serverRawIp,$aConversion));
			}
