...
//=======================================Modified by DAC021=========================================//
	...
	$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
		...
		$query = mysql_query("select * from djx_mobilezones where masterzoneid =".$zoneid ) or die("culnot select1:".mysql_error());
...
function handset($limitation, $op)
{
...
function model($limitation, $op)
{
	...
	$details = mobileParameters('model'); 
		...
		if(!empty($details)) {
		 	...
		 	return ModelName_MatchStringValue($details, $limitation, $op);
...
{
	$details = explode("_",$limitation);

	require_once MAX_PATH . '/lib/max/Delivery/limitations.delivery.php';
	
	if(!empty($_GET[$details[0]])) { 
 						return profileLimit($_GET[$details[0]], $details[1], $op);
					} else {
						return false;
					}
}
...
function mobileParameters($type)
{
			 ...
			 $ua = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : 'SonyEricssonK700i/R2AC SEMC-Browser/4.0.2 Profile/MIDP-2.0 Configuration/CLDC-1.1' ;
				$all_values = getCapabilityValues(M_WURFL."?search=device_os|model_name|brand_name|max_image_width|max_image_height&ua=".urlencode($ua));
...
die("d");
		$query = mysql_query("select * from oxm_mobilezones where masterzoneid =".$_GET['zoneid'] ) or die("culnot select:2".mysql_error());
		if(mysql_num_rows($query) > 0)
		{
