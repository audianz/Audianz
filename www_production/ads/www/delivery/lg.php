...
/* To store target impressions corr. to mix_id */

function store_target_impressions($campid)
{
	global $log;
	$date=date("Y-m-d");
	...
	{
		$date		=	date("Y-m-d");
		$query		=	mysql_query("Select * from Campaign_Target_Impressions_Mix  where Campaign_id='$campid' and Mix_id=( SELECT Mix_id FROM Campaign_Optimize_Mix WHERE Network ='smaato') ");
		
		if(mysql_num_rows($query)>0)
		{
			$res		=	mysql_fetch_array($query);
			$imp		=	$res['Impressions_served'];
			$imp		=	$imp+1;
			mysql_query("Update Campaign_Target_Impressions_Mix  set Impressions_served='$imp' where Campaign_id='$campid' and Mix_id=( SELECT Mix_id FROM Campaign_Optimize_Mix WHERE Network ='smaato')  ");
		}
...
{
	global $log;
//        $log->logDebug("log impression zone is ".$zoneid);
	...
	if (_viewersHostOkayToLog())
	{
		...
		$aConf = $GLOBALS['_MAX']['CONF'];
		//Added by neha  on 19-Sep-2013
		if($zoneid!=0)
		{	
			//$log->logDebug("log impression zone is ".$zoneid);
			$array_val=array("zoneid"=>$zoneid,"clientid"=>$clientid,"campid"=>$campaignid,"bannerid"=>$bannerid,"poiId"=>$_GET[$aConf['var']['POI']]);
		...
		if($revenue_type==$revenue_value)
		{
			...
			if(mysql_num_rows($account_value)>0)
			{
				...
				$perimpression =$perclick1['revenue']/1000;
				$acc_amount = $accamount -$perimpression ;
				$bud_amount = $budamount -$perimpression ;
				$date=date("Y-m-d");
				$monthvalue=substr($date,5,2);
				$amount=$amount+$perimpression;
				$admin_commission=(($commission/100)*$perimpression);
				$pub_commission=$perimpression-$admin_commission;
		...
		else
		{
			...
			if($_GET['zoneid']==0) {
				...
				$rateimp = $_REQUEST['banner_rate']/1000;				
				mysql_query("UPDATE oxm_accbalance SET accbalance=accbalance-".$rateimp." 
					     WHERE clientid=".$clientid."") or die(mysql_error());


				//$log->logDebug("log impression price is ".$price."   rate is ".$_REQUEST['banner_rate']);	
				$array_val=array("zoneid"=>0,"clientid"=>$clientid,"campid"=>$campaignid,"bannerid"=>$adid,"poiId"=>$poiId);
				log_Store_Impression($array_val);
				store_target_impressions($campaignid);
				...
				$query=mysql_query("select * from oxm_report where clientid=".$clientid." AND zoneid=0 AND bannerid=".$adid." AND pricingmodel=1 AND date='$date'");
					
				if(mysql_num_rows($query)>0)
				{
					$totalvalue	 =	mysql_fetch_array($query);
					$total		 =	$totalvalue['amount'] + $_REQUEST['banner_rate']/1000;
					$admin_comm	 =	($_REQUEST['banner_rate'] - $_REQUEST['price'])/1000;
					$admin_commission=	$totalvalue['publisher_amount'] + $admin_comm;
					$date		 =	date("Y-m-d");
					$year		 =	substr($date,0,4);
					$month		 =	substr($date,5,2);
					//				$total					=	$tvalue+$perimpression;
					//					$admin_commission		=	$pubvalue+$pub_commission;
						
					mysql_query("update oxm_report set amount=".$total.",publisher_amount=".$admin_commission."
				...
				else
				{
					...
					$admin_commission 		=($_REQUEST['banner_rate'] - $_REQUEST['price'])/1000;
					$amount				=$_REQUEST['banner_rate']/1000;
					mysql_query("insert into oxm_report(clientid,zoneid,campaignid,bannerid,amount,date,pricingmodel,publisherid,publisher_amount)values(".$clientid.",0,".$campaignid.",".$adid.",".$amount.",'".$date."',1,0,".$admin_commission.")") or die(mysql_error());
