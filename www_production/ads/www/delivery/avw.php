...
$log->logInfo("Entered avw.php");

$poiIndx ;
$poiIndxCurrentBannerId = 0;
$location_limitation = 0;
...
/** This function is used to store the impression data
 *  of corresponding storefront
 * @author SHYAM
 * @param  $arr
 */

function log_Store_Impression($arr)
{

	$date=date("Y-m-d");
	$con = mysql_connect($GLOBALS['_MAX']['CONF']['database']['host'],
			$GLOBALS['_MAX']['CONF']['database']['username'],
	...
	if(!empty($arr))
	{
		...
		$zoneid		=	$arr['zoneid'];
		$campid		=	$arr['campid'];
		$bannerid	=	$arr['bannerid'];
		$poiId		=	$arr['poiId'];

		$query		=	mysql_query("select * from oxm_store_report where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId'  AND date(date)='$date'");
		...
		{
			$imp	=	$imp+1;
			mysql_query("update oxm_store_report set impressions='$imp' where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId' AND date(date)='$date'");
		}
		else
		{
			$imp	=	1;
			mysql_query("insert into oxm_store_report(clientid,zoneid,campaignid,bannerid,date,poi_id,impressions)values('$clientid','$zoneid','$campid','$bannerid','$date','$poiId','$imp')") or die(mysql_error());
...
function MAX_Delivery_log_logAdImpression($adId, $zoneId)
{
	...
	$log->logDebug("Entered MAX_Delivery_log_logAdImpression() and adId is ".$adId." zoneid is ".$zoneId);
	...
	if (_viewersHostOkayToLog())
	{
		...
		$banner_click=mysql_query("SELECT b.campaignid,c.campaignid,b.bannerid,z.ad_id,c.clientid,s.clientid,o.zoneid,o.revenue_type,o.affiliateid,z.zone_id,c.revenue,c.revenue_type FROM ox_banners b, ox_ad_zone_assoc z, ox_campaigns c, ox_zones o,ox_clients s  WHERE b.bannerid = z.ad_id AND c.campaignid = b.campaignid AND c.revenue_type=o.revenue_type AND s.clientid=c.clientid AND b.bannerid=".$adId." AND o.zoneid =".$zoneId."") or die("mysqlerror");
		$ban_click=mysql_fetch_array($banner_click);
		$zoneid=$ban_click['zoneid'];
		$clientid=$ban_click['clientid'];
		$campaignid=$ban_click['campaignid'];
		$bannerid=$ban_click['ad_id'];
		$revenue_type=$ban_click['revenue_type'];
		$affiliateid=$ban_click['affiliateid'];



		$aff_com=mysql_query("select * from ox_affiliates where affiliateid='$affiliateid'");
		$af_com=mysql_fetch_array($aff_com);
		if($af_com['publishershare']!="" && $af_com['publishershare']!=0)
		{
			$tot_pubshare=100;
			$commission=$tot_pubshare-$af_com['publishershare'];
		}
		else
		{
			$tot_pubshare=100;
			$pub_share=mysql_query("select * from oxm_publisher_share where id='1'");
			$pub_comm=mysql_fetch_array($pub_share);
			$commission=$tot_pubshare-$pub_comm['publishershare'];
		}

		$revenue_value=1;
		if($revenue_type==$revenue_value)
		{
			$account_value = mysql_query("select * from oxm_accbalance where clientid=".$clientid."");
			$budget_value=mysql_query("select * from oxm_budget where clientid=".$clientid." AND campaignid=".$campaignid."");
			if(mysql_num_rows($account_value)>0)
			{
				$account_val = mysql_fetch_array($account_value);
				$budget_val=mysql_fetch_array($budget_value);
				$accamount = $account_val['accbalance'];
				$acc_mail_status = $account_val['mail_status'];//DAC016
				$budamount=$budget_val['budget'];
				$bud_dailymail_status=$budget_val['dailymail_status'];//DAC016
				$perclicks=mysql_query("select * from ox_campaigns WHERE campaignid=".$campaignid."") or ("mysql_error()");
				$perclick1=mysql_fetch_array($perclicks);
				$perclick =$perclick1['revenue']/1000;
					
				$acc_amount = $accamount -$perclick ;
				$bud_amount = $budamount -$perclick ;
				$date=date("Y-m-d");
				$monthvalue=substr($date,5,2);
				$amount=$amount+$perclick;
				$admin_commission=(($commission/100)*$perclick);
				$pub_commission=$perclick-$admin_commission;

				$currentdate=$budget_val['currentdate'];
				$dailybudget=$budget_val['dailybudget'];
				if($currentdate!=$date)
				{
					if($accamount > $budamount)
					{
						//mysql_query("update oxm_budget set budget='$dailybudget',currentdate='$date' where campaignid='$campaignid'");
					}
				}
				if(($accamount>=$perclick))
				{
					if($logImpression)
					{
						OX_Delivery_Common_hook('logImpression', array($adId, $zoneId));

						mysql_query("update oxm_accbalance set accbalance=".$acc_amount." where accbalance>0 AND  clientid=".$clientid."") or 					die(mysql_error());
						mysql_query("update oxm_budget set budget=".$bud_amount." where clientid=".$clientid." AND campaignid=".$campaignid."") or die(mysql_error());

						$query=mysql_query("select * from oxm_report where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND pricingmodel='$revenue_type'AND date='$date'");
						$totalvalue=mysql_fetch_array($query);
						$tvalue=$totalvalue['amount'];
						$pubvalue=$totalvalue['publisher_amount'];
						$date=date("Y-m-d");
						...
						if(mysql_num_rows($query)>0)
						{
							$total=$tvalue+$perclick;
							$publisher_commission=$pubvalue+$pub_commission;
							mysql_query("update oxm_report set amount='$total',publisher_amount='$publisher_commission' where clientid='$clientid'AND zoneid='$zoneid' AND bannerid='$bannerid'AND pricingmodel='$revenue_type' AND date='$date'");

						}
						else
						{
							mysql_query("insert into oxm_report(clientid,zoneid,campaignid,bannerid,amount,date,pricingmodel,publisherid,publisher_amount)values('$clientid','$zoneid','$campaignid','$bannerid','$amount','$date','$revenue_type','$affiliateid','$pub_commission')") or die(mysql_error());

						}

						/////RTB start

						$aZone = OA_Dal_Delivery_getZoneInfo($zoneId);
						...
						if($aZone['rtb']==1){
							...
							$tardet1=mysql_query("select * from aff_rtb where campaign_id in(".$value.") and bid_rate!='0'")or die("error1".mysql_error());
							while($fetchbidvalue=mysql_fetch_array($tardet1))
							{
								$target="";


								$detection=mysql_fetch_array(mysql_query("SELECT c.clientid,d.accbalance,a.revenue_type FROM `oxm_accbalance` as d,ox_banners as b,ox_clients as c,ox_campaigns as a where c.clientid=a.clientid and a.campaignid=b.campaignid and d.clientid=c.clientid and b.bannerid=".$adId))or die(mysql_error());

								///getcamp name


								$getcampname=mysql_fetch_array(mysql_query("select * from ox_campaigns where campaignid='".$fetchbidvalue['campaign_id']."'"))or die(mysql_error());

								$_campname=$getcampname['campaignname'];


								$locurl=$GLOBALS['loc'];

								$bidrate=$fetchbidvalue['bid_rate'];
								$compadv=$_campname.'-'.$bidrate;
								///Country and keyword -----Update biding statitics

								if($fetchbidvalue['type']=='country')
								{

									$allcountry=explode(',',$fetchbidvalue['data']);
									$countrycode=$GLOBALS['_MAX']['CLIENT_GEO']['country_code'];

									if(in_array($countrycode,$allcountry))
									{
											
										$target='country**'.$GLOBALS['_MAX']['CLIENT_GEO']['country_code'].'-*'.$fetchbidvalue['comparison'];
											
										////rtbstat insert

										$exdat=mysql_query("select * from aff_rtb_statistics where date='".date('Y-m-d')."' and bid_win_amount=".$bidrate." and match_campaign='".$compadv."' and banner_id=".$adId." and zone_id=".$zoneid." and win_criteria='".$target."'")or die(mysql_error());
										$valdet=mysql_fetch_array($exdat);

											

										if($detection['revenue_type']==1)
										{
											$total=$valdet['amount']+($bidrate/1000);
											$bidval=($bidrate/1000);

											if(mysql_num_rows($exdat)<=0)
											{

												mysql_query("INSERT INTO aff_rtb_statistics(date,banner_id,zone_id,bid_win_amount,win_criteria,match_campaign,impressions,clicks,amount) VALUES ('".date('Y-m-d')."', ".$adId.", ".$zoneid.",".$bidrate.", '".$target."', '".$compadv."', '1', '0','$bidval')")or die(mysql_error());
											}
											else
											{

												$imp=$valdet['impressions']+1;

												mysql_query("UPDATE `aff_rtb_statistics` SET `impressions` = ".$imp.",amount='".$total."' where date='".date('Y-m-d')."' and bid_win_amount=".$bidrate." and match_campaign='".$compadv."' and banner_id=".$adId." and zone_id=".$zoneid." and win_criteria='".$target."' ")or die(mysql_error());
								...
								///End Country and keyword -----Update biding statitics
								///////Updation of advimpr table
			...
			else if(($accamount<=0) && ($acc_mail_status != '1'))
			{
				$queryadvertiser=mysql_query("select * from ox_campaigns where campaignid=".$campaignid."") or die("mysql_error()");
				$norow=mysql_fetch_array($queryadvertiser);
				$mailid=mysql_query("select * from ox_clients where clientid=".$clientid."") or die("mysql_error()");
				$norow1=mysql_fetch_array($mailid);
				$to=$norow1['email'];

				///dac015///

				$query_ad=mysql_query("select * from  oxm_mail where mailtype='3'");
				$queryad=mysql_fetch_array($query_ad);
				$subject=$queryad['subject'];
				$message=nl2br($queryad['message']);
				...
				mysql_query("UPDATE oxm_accbalance SET mail_status = '1' WHERE clientid =".$clientid)or die('error in Update');


				/**************************** Modified BY DAC016 **********************************/
			}
			else if(($budamount<=0) && ($bud_dailymail_status != '1'))
			{
				$queryadvertiser=mysql_query("select * from ox_campaigns where campaignid=".$campaignid."") or die("mysql_error()");
				$norow=mysql_fetch_array($queryadvertiser);
				$mailid=mysql_query("select * from ox_clients where clientid=".$clientid."") or die("mysql_error()");
				...
				$query_ad=mysql_query("select * from  oxm_mail where mailtype='7'");
				$queryad=mysql_fetch_array($query_ad);
				$subject=$queryad['subject'];
				$message=nl2br($queryad['message']);
				$headers  = 'MIME-Version: 1.0' . "\r\n";
				...
				mysql_query("UPDATE oxm_budget SET dailymail_status = '1' WHERE clientid =".$clientid)or die('error in Update');
						...
						if(in_array($countrycode,$allcountry))
						{
							...
							if($detection['revenue_type']!=1)
							{
								...
								if(mysql_num_rows($exdat)<=0)
								{
									...
									mysql_query("INSERT INTO aff_rtb_statistics(date,banner_id,zone_id,bid_win_amount,win_criteria,match_campaign,impressions,clicks) VALUES ('".date('Y-m-d')."', ".$adId.", ".$zoneid.",".$bidrate.", '".$target."', '".$compadv."', '1', '0')")or die(mysql_error());
								}
								else
								{
									...
									mysql_query("UPDATE `aff_rtb_statistics` SET `impressions` = ".$imp." where date='".date('Y-m-d')."' and bid_win_amount=".$bidrate." and match_campaign='".$compadv."' and banner_id=".$adId." and zone_id=".$zoneid." and win_criteria='".$target."' ")or die(mysql_error());
		...
		global $poiIndx;
		$poiId=0;;
		foreach ($poiIndx as $poi)
		{
			if($poi['adId']==$bannerid)
			{
				$poiId=$poi['poiId'];
				break;
			}
		...
		$array_val=array("zoneid"=>$zoneid,"clientid"=>$clientid,"campid"=>$campaignid,"bannerid"=>$bannerid,"poiId"=>$poiId);

		log_Store_Impression($array_val);
...
function MAX_commonInitVariables()
{
	...
	$query=mysql_query("select * from ".$table_prefix."zones where zoneid =".$zoneid ) or die("culnot select1:".mysql_error());
	$zone_det=mysql_fetch_array($query);


	require_once 'handset.php';
	if($zone_det['master_zone'] == '-2')
	{
		$zoneid = changeZone($zoneid);
...
function MAX_redirect($url)
{
	...
	if (!preg_match('/^(?:javascript|data):/i', $url)) {
		header('Location: '.$url) or die("dsr");
		MAX_sendStatusCode(200);
	}
}
...
function mobiLimit($row, $source = '')
{
	/*****************************/
	$cid    =$row['placement_id'];
	$sql    ="SELECT compiledlimitation, acl_plugins FROM djx_campaign_limitation WHERE campaignid=".$cid;
	$exec   =mysql_query($sql);
	$result =mysql_fetch_array($exec);
	/****************************/

	$row['compiledlimitation'] =$result['compiledlimitation'];
	$row['acl_plugins']        =$result['acl_plugins'];
	$row['campaignid']	=$cid ;
	...
	if (!empty($row['compiledlimitation'])) {
		...
		if(strlen($row['acl_plugins'])) {
			$acl_plugins = explode(',', $row['acl_plugins']);
			foreach ($acl_plugins as $acl_plugin) {
				if (!isset($GLOBALS['_MAX']['FILES']['aIncludedPlugins'][$acl_plugin])) {

					if ($acl_plugin) {
						$GLOBALS['_MAX']['FILES']['aIncludedPlugins'][$acl_plugin] = true;
					} else {
						return true;
					}
				}
...
/////DAC015
function MAX_checkMobileCarrierLimitation_mobileCarrierLimitation($limitation, $op) //Mobile carrier
{
	...
	$del = mysql_query("SELECT * FROM ox_carrier_detail WHERE INET_ATON(start_ip) <= INET_ATON('".$ip."') AND INET_ATON(end_ip) >= INET_ATON('".$ip."')")or die('Not ');
...
//added by neha
function device_location($limitation, $op)
{
	global $log;
	global $location_limitation;
	$location_limitation = 1;
	return true;
}

function redirect($limitation, $op)
{
	return true;
}
//added by neha
function intermediate_landing($limitation, $op)
{
	return true;
}

/////DAC015
function device_manufacturer($limitation, $op) // handset
{
	require_once MAX_PATH . '/lib/max/Delivery/limitations.delivery.php';

	$details = mobileParameters('handset');

	if(!empty($details)) {

		return MAX_limitationsMatchArrayValue($details, $limitation, $op);
...
function device_os($limitation, $op)
{

	require_once MAX_PATH . '/lib/max/Delivery/limitations.delivery.php';

	$details = mobileParameters('os');


	$handset = mobileParameters('handset');

	if($handset == "generic web browser") {
...
function geographic_operators($limitation, $op)
{
	require_once MAX_PATH . '/lib/max/Delivery/limitations.delivery.php';
	...
	if(!empty($details)) {
		...
		$_details = explode(",",$details);


		$count = count($_details);
		...
		if($count > 1) {
			...
			for($v=0; $v < $count; $v++) {
				...
				$return = MAX_limitationsMatchArrayValue($_details[$v], $limitation, $op);
...
function device_capability($limitation, $op) // handset
{

	require_once MAX_PATH . '/lib/max/Delivery/limitations.delivery.php';


	$details = mobileParameters('capability');


	if(!empty($details)) {

		return MAX_limitationsMatchArrayValue1($details, $limitation, $op);
...
function geographic_locations($limitation, $op)
{

	require_once MAX_PATH . '/lib/max/Delivery/limitations.delivery.php';

	$details = $GLOBALS['_MAX']['CLIENT_GEO']['country_code'];
...
function model($limitation, $op)
{
	...
	$details = mobileParameters('model');

	if(!empty($details)) {

		return Model_limitationsMatchStringValue($details, $limitation, $op);
...
function demographics($limitation, $op)
{
	$limit    	 =explode("|",$limitation);
	$db_gender   =$limit[0]; // all | m | f
	$db_age      =$limit[1]; // 35-44,45-54,55-64


	$url_gender   =($_GET['gender'] == '')?'all':$_GET['gender'];

	$url_age      =($_GET['age'] == '')?'a':$_GET['age'];

	//$url_gender   =$_GET['gender'];

	//$url_age      =$_GET['age'];
	if($db_gender=="all" && $db_age=="a" && $url_gender=="" && $url_age=="" )
	{
		return true;
	}

	else if($db_gender=="all" && $db_age=="a" && $url_gender == "all" && $url_age == "a")
	{

		return true;
	}
	else if ($url_gender!="all" && $url_age == "a")
	{


		if($url_gender==$db_gender)
		{
			return true;
		}
	...
	elseif($url_gender=="all" && $url_age != "a")
	{

		$_age =explode(",",$db_age);
		$age =false;
		for($i=0;$i<count($_age);$i++)
		{
			$x       =explode("-", $_age[$i]);
			$low     =$x[0];
			...
			if($url_age >=$low && $url_age <= $upper && $url_gender=="all") {
	...
	elseif($url_gender!="all" && $url_age != "a")
	{
		$_age =explode(",",$db_age);
		$age =false;
		if($url_gender==$db_gender && $_age[0]=="a" )
		{
			return true;
		}
		else
		{
			for($i=0;$i<count($_age);$i++)
			{
				...
				if($url_age >=$low && $url_age <= $upper && $url_gender==$db_gender) {
...
function mobileParameters($type)
{
	...
	if($type == 'handset') {
		$hanset_brand = $all[2]['value'];  //device Brand
		return $hanset_brand;
	} elseif($type == 'os') {

		$hanset_os = $all[0]['value'];   //device OS
		return $hanset_os;
	} elseif($type == 'model') {
		$hanset_model = $all[1]['value'];  //device Model
		return $hanset_model;
	} elseif($type='capability') {
		if($all[6]['value']=="false"){
			$hanset_capability[] = $all[6]['value'];
		}else{$hanset_capability[]="j2me_midp_1_0";
		}
		if($all[7]['value']=="false"){
			$hanset_capability[]= $all[7]['value'];
		}else{$hanset_capability[]="j2me_midp_2_0";
		}
		if($all[8]['value']=="false"){
			$hanset_capability[] = $all[8]['value'];
		}else{$hanset_capability[]="fl_browser";
		}
		if($all[9]['value']=="false")
		{
			$hanset_capability[] = $all[9]['value'];
		}
		else
		{$hanset_capability[]="progressive_download";
		}  //device capability
		return $hanset_capability;
	}
...
function _adRenderBuildClickUrl($aBanner, $zoneId=0, $source='', $ct0='', $logClick=true, $overrideDest=false)
{
	...
	if (is_string($logClick)) {
		...
		//added by neha need to review.
		global $poiIndx;
		$currentPoiId = -1;

/*		if($poiIndxCurrentBannerId != 0)
		{
			for($i = 0; $i < count($poiIndx); $i++)
			{
				if(($poiIndx[$i]['adId'] == $poiIndxCurrentBannerId) && ($poiIndx[$i]['adId'] == $aBanner['ad_id']))
				{
					if(!empty($poiIndx[$i]['poiId']) && ($poiIndx[$i]['poiId'] != -1))
					{
							
						$currentPoiId = $poiIndx[$i]['poiId'];
						$log->logDebug("avw : poi's current value is ".$currentPoiId);
						break;
					}
				}
			}
		...
		{
			$log->logDebug("avw banner id not yet set.");
		}
*/
		for($i = 0; $i < count($poiIndx); $i++)
			{
				if($poiIndx[$i]['adId'] == $aBanner['ad_id'])
				{
					if(!empty($poiIndx[$i]['poiId']) && ($poiIndx[$i]['poiId'] != -1))
					{
		...
		$clickUrl .= "&amp;".$conf['var']['lat']."=".$_GET['lat']."&amp;".$conf['var']['lon']."=".$_GET['lon']."&amp;".$conf['var']['POIKey']."=".$currentPoiId."&amp;type=3";
	...
	foreach($aAds as $aLinkedAd) {
		if (!empty($aLinkedAd['priority'])) {
			...
			if ($high > $random_num && $low <= $random_num) {
				...
				$query = mysql_query("select campaignid, clientid,revenue,revenue_type from ox_campaigns  where campaignid = ".$aLinkedAd['placement_id']." ") or die("Error1".mysql_error());

				$row = mysql_fetch_assoc($query);

				$clientt_id = $row['clientid'];
				...
				$acc_balance=mysql_query("select * from oxm_accbalance where clientid='$clientt_id'");


				$acc_bal=mysql_fetch_array($acc_balance);

				$acc_bala=$acc_bal['accbalance'];
				$acc_mail_status=$acc_bal['mail_status'];//DAC016


				/*Daily budget */
				$dailybud=mysql_query("select * from oxm_budget where clientid='$clientt_id' AND campaignid=".$aLinkedAd['placement_id']."");
				$daily_bal=mysql_fetch_array($dailybud);

				$dailybudget=$daily_bal['dailybudget'];


				/*total amount */
				$date=date("Y-m-d");
				$totalamount=mysql_query("select sum(amount) from oxm_report where clientid='$clientt_id' AND campaignid=".$aLinkedAd['placement_id']." AND date='$date'");
				$tot_amount=mysql_fetch_array($totalamount);

				$total_amount=$tot_amount['sum(amount)'];

				$zonevalue=mysql_query("select * from ox_zones where zoneid=".$aLinkedAd['zone_id']."");

				$zoneamount=mysql_fetch_array($zonevalue);

				$pubid=$zoneamount['affiliateid'];



				/*Fetch name from ox_clients */
				$ox_cli=mysql_query("select * from ox_clients where clientid='$clientt_id'");
				$ox_client=mysql_fetch_array($ox_cli);
				$clientname=$ox_client['clientname'];



				/*Send mail to advertiser & publisher */
				/*Fetch daily bal & Acc balance from  oxm_notification */
				/*Clientd id */
				$admindetail=mysql_query("select * from oxm_admindetails where id='1'");
				$admin_detail=mysql_fetch_array($admindetail);
				$adminemail=$admin_detail['Email'];


				$date=date("Y-m-d");
				$oxm_notify=mysql_query("select * from  oxm_notification where clientid='$clientt_id'");
				if(mysql_num_rows($oxm_notify)>0)
				{
					$oxm_notfy=mysql_fetch_array($oxm_notify);
					$nf_clientid=$oxm_notfy['clientid'];
					$nf_accmail_status=$oxm_notfy['accmail_status'];
					$nf_budmail_status=$oxm_notfy['budmail_status'];

					$nf_adv_accbal=$oxm_notfy['accbalance'];
					$nf_ad_acc=explode(",",$nf_adv_accbal);
					$final_adv_accbal=$nf_ad_acc[1];
					$nf_adv_budbal=$oxm_notfy['budbalance'];
					$nf_ad_bud=explode(",",$nf_adv_budbal);
					$final_adv_budbal=$nf_ad_bud[1];


					$oxm_adv_acc=mysql_query("select sum(amount) as Amount from oxm_report where clientid='$clientt_id' AND date='$date'");
					$oxm_adv_ac=mysql_fetch_array($oxm_adv_acc);
					$adv_amount=$oxm_adv_ac['Amount'];
					$adv_amounts=$dailybudget-$adv_amount;

					$oxm_acc=mysql_query("select * from oxm_accbalance where clientid='$clientt_id'");
					$oxm_ac=mysql_fetch_array($oxm_acc);
					$oxm_acc_amount=$oxm_ac['accbalance'];

					$email1=mysql_query("select * from ox_clients where clientid='$clientt_id'");
					$email_1=mysql_fetch_array($email1);
					$nf_adv_email=$email_1['email'];



					/*Daily Budget send mail to Adv & admin */


					if($final_adv_budbal >=$adv_amounts && $nf_budmail_status != '1' && $nf_ad_bud[0]=='yes')
					{
						$to=$nf_adv_email;
						$query_ad=mysql_query("select * from  oxm_mail where mailtype='4'");
						$queryad=mysql_fetch_array($query_ad);
						$subject=$queryad['subject'];
						$msg=nl2br($queryad['message']);
						...
						$message .= "<tr><td>"."Client Name:".$clientname."</td></tr>";
						$message .="<tr><td height='34'>Your Daily budget reached to $".$final_adv_budbal." dated on ".$date."</td></tr>";
						$message .= "</body></html>";$message .= "</table>";
						...
						mysql_query("UPDATE oxm_notification SET budmail_status = '1' WHERE clientid  =".$clientt_id)or die('error in Update');
					...
					if($final_adv_accbal>=$oxm_acc_amount && $nf_accmail_status != '1' && $nf_ad_acc[0]=='yes')
					{
						$to=$nf_adv_email;
						...
						$message .= "<tr><td>".$msg."</td></tr>";
						$message .= "<tr><td>"."Client Name:".$clientname."</td></tr>";
						$message .="<tr><td height='34'>Your AccountBalance reached to $".$final_adv_accbal." dated on ".$date."</td></tr>";
						$message .= "</body></html>";$message .= "</table>";
						...
						mysql_query("UPDATE oxm_notification SET accmail_status = '1' WHERE clientid =".$clientt_id)or die('error in Update');
				...
				/*Publisher Id */
				$oxm_notify_pub=mysql_query("select * from  oxm_notification where publisherid='$pubid'");
				if(mysql_num_rows($oxm_notify_pub)>0)
				{
					$oxm_notfy_pub=mysql_fetch_array($oxm_notify_pub);
					$nf_publisherid=$oxm_notfy_pub['publisherid'];
					$nf_accmail_status=$oxm_notfy_pub['accmail_status'];
					$nf_budmail_status=$oxm_notfy_pub['budmail_status'];
					$nf_pub_accbal=$oxm_notfy_pub['accbalance'];
					$nf_pub_acc=explode(",",$nf_pub_accbal);
					$final_pub_accbal=$nf_pub_acc[1];
					$nf_pub_budbal=$oxm_notfy_pub['budbalance'];
					$nf_pub_bud=explode(",",$nf_pub_budbal);
					$final_pub_bud=$nf_pub_bud[1];



					/*This query for daily budget */

					$oxm_pub_acc=mysql_query("select sum(publisher_amount) as publisher_amount  from oxm_report where publisherid='$pubid' AND date='$date'");
					$oxm_pub_ac=mysql_fetch_array($oxm_pub_acc);
					$pub_amount=$oxm_pub_ac['publisher_amount'];

					/*This query for Total Revenue */
					$oxm_pub_tot=mysql_query("select sum(publisher_amount) from oxm_report where publisherid='$pubid'");
					$oxm_pub_tt=mysql_fetch_array($oxm_pub_tot);
					$tot_amount=$oxm_pub_tt['sum(publisher_amount)'];

					$aff_com=mysql_query("select * from ox_affiliates where affiliateid='$pubid'");
					$af_com=mysql_fetch_array($aff_com);
					$pubname=$af_com['contact'];
					$nf_pub_email=$af_com['email'];
					/*Publisher Daily Revenue */


					if($final_pub_bud <= $pub_amount && $nf_budmail_status !='1' && $nf_pub_bud[0]=="yes" )
					{
						...
						$message .= '<table width="70%" style="border-color: rgb(102, 102, 102); background: none repeat scroll 0% 0% rgb(226, 226, 226);"  cellspacing="10" cellpadding="10" >';
						$message .= "<tr><td>".$msg."</td></tr>";
						$message .= "<tr><td>"."Publisher Name:".$pubname."</td></tr>";
						$message .="<tr><td height='34'>Your Daily Revenue reached to $".$final_pub_bud." dated on ".$date."</td></tr>";
						$message .= "</body></html>";$message .= "</table>";
						...
						mysql_query("UPDATE oxm_notification SET budmail_status = '1' WHERE publisherid  =".$pubid)or die('error in Update');
					...
					/*Pub-Total Revenue */
					if($final_pub_accbal <= $tot_amount && $nf_accmail_status !='1' && $nf_pub_acc[0]=="yes")
					{
						$to=$nf_pub_email;

						$query_ad=mysql_query("select * from  oxm_mail where mailtype='6'");
						$queryad=mysql_fetch_array($query_ad);
						$subject=$queryad['subject'];
						$msg=nl2br($queryad['message']);
						...
						$message .= "<tr><td>".$msg."</td></tr>";
						$message .= "<tr><td>"."Publisher Name:".$pubname."</td></tr>";
						$message .="<tr><td height='34'>Your Daily Revenue reached to $".$final_pub_accbal." dated on ".$date."</td></tr>";
						$message .= "</body></html>";$message .= "</table>";
						...
						mysql_query("UPDATE oxm_notification SET accmail_status = '1' WHERE publisherid  =".$pubid)or die('error in Update');

						$nf_accmail_status ='1' ;
						/**************************** Modified BY DAC016 **********************************/
					}
				}
				$rm=$dailybudget-$total_amount;
				if(($dailybudget>=$total_amount) && ($acc_bala>0) && ($acc_bala>=$cam_amount) && ($rm>=$cam_amount) ) {
...
function _adSelectCheckCriteria($aAd, $aContext, $source, $richMedia)
{
	...
	$query = mysql_query("select master_zone from ox_zones where (master_zone=-1 OR master_zone=-2) AND (zoneid ='".$_GET['zoneid']."')") or die("culnot select:2".mysql_error());
	if(mysql_num_rows($query) > 0)
	{
		//==========Mobile Delivery Limitations========//
			
		$_campaignQuery = mysql_query("select * from djx_campaign_limitation where campaignid=".$aAd['placement_id']) or die("Errorn In Campaign Limitations--->".mysql_error());
		$_campaignValues = mysql_fetch_assoc($_campaignQuery);

		if($_campaignValues['compiledlimitation'] != '')
		{
			if(mobiLimit($aAd, $source) == false) {
				return false;
			}
		}
	...
	else
	{
		...
		if($_campaignValues['compiledlimitation'] != '')
		{
			if (strpos($_campaignValues['compiledlimitation'],'device_location') !== false)
			{
				device_location();
			}
		}
	...
	//=============================== Modified by DAC016 =========================================//
	$query1 = mysql_query("select catagory from ".$table_prefix."campaigns where campaignid=".$aAd['placement_id']) or die("culnot select:3".mysql_error());

	$_campaign = mysql_fetch_assoc($query1);

	$_category = explode(',',$_campaign['catagory']);

	$query2 = mysql_query("select a.oac_category_id  from ".$table_prefix."affiliates as a, ".$table_prefix."zones as b where b.affiliateid = a.affiliateid and  b.zoneid = ".$_GET['zoneid']." ") or die("culnot select:5".mysql_error());

	$_Zone = mysql_fetch_assoc($query2);

	$_Zone_category = explode(',',$_Zone['oac_category_id']);
	...
	if(mysql_num_rows($query1) > 0)
	{
		...
		if(mysql_num_rows($query2) > 0){
			...
			foreach($_category as $key => $value){
				...
				if(in_array($value,$_Zone_category)){
	...
	$_zonequr= mysql_query("SELECT * FROM ox_zones WHERE zoneid='".$_GET['zoneid']."'");
	$aZone=mysql_fetch_array($_zonequr);
	if($aZone['rtb']==1){



		$queryPublisherval = mysql_query("SELECT pb.pubid,pb.bid from ".$table_prefix."zones as z,  oxm_pub_bidding as pb where z.zoneid = '".$_GET['zoneid']."' AND z.affiliateid = pb.pubid") or die("Errew3".mysql_error());
		$rowCheck = mysql_fetch_assoc($queryPublisherval);
		$bidrate = $rowCheck['bid'];


		$_rev=mysql_query("SELECT * FROM ox_campaigns WHERE campaignid='".$aAd['placement_id']."'")or die("asew");
		$_fetchrev=mysql_fetch_assoc($_rev);

		$camp_rev=$_fetchrev['revenue'];
	...
	//Added by neha
	//if it comes here
	//if(global val is true call fun)
	global $location_limitation;
	if($location_limitation == 1)
	{
		$location_limitation = 0;
		$result = _adLocationMatches($aAd['ad_id'], $aAd['client_id']);
		if($result == false)
		{
			$log->logDebug("** adSelectCheckCriteria return false \n");
		}
		return $result;
	}
	else if($location_limitation == 0)
	{
		$log->logDebug(" avw location_limitation == 0 \n");
	}
	return true;
}
...
//added by neha
function  _adLocationMatches($adId, $clientId)
{
	global $log;

	$conf = $GLOBALS['_MAX']['CONF'];
	$con = mysql_connect($GLOBALS['_MAX']['CONF']['database']['host'],$GLOBALS['_MAX']['CONF']['database']['username'],$GLOBALS['_MAX']['CONF']['database']['password']);
	mysql_select_db($GLOBALS['_MAX']['CONF']['database']['name'], $con)or die("culnot selectn:".mysql_error());
	$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
	$_distanceQuery = mysql_query("select * from storefrontdata where (clientid='".$clientId."')") or die("culnot select:n".mysql_error());
	$result = false;
	if(mysql_num_rows($_distanceQuery) > 0)
	{
		...
		$_radiusQuery = mysql_query("select * from storefront_radius where (advid='".$clientId."')") or die("culnot select:n".mysql_error());
	
		if(mysql_num_rows($_radiusQuery) > 0)
		{
			while ($radiusRow = mysql_fetch_array($_radiusQuery, MYSQL_BOTH))
			{
				$radiusVal = $radiusRow['radius'];
			}
				
		}
		while ($row = mysql_fetch_array($_distanceQuery, MYSQL_BOTH))
		{
			$count=$count+1;

			$dis = distance($row['lat'], $row['lon'], $_GET['lat'], $_GET['lon'], k) ;
	...
	/*		$radiusVal = 5;
			...
			mysql_free_result($_radiusQuery); */
			if(($dis >= 0) && ($dis <= $radiusVal) )
			{
				$temp['poiId'] = $row['id'];
				$temp['distance'] = $dis;
	...
	{
		$log->logDebug("adLocationMatches storefront row not found \n");
	}
	mysql_free_result($_distanceQuery);
	if($result == false)
	{
		$log->logDebug("** adLocationMatches return false \n");
	}
	return $result;
}
...
function _adSelectDiscardNonMatchingAds(&$aAds, $aContext, $source, $richMedia)
{
	...
	foreach ($aAds as $adId => $aAd) {
		...
		//	$log->logDebug("failed adSelectCheckCriteria for banner id :".$aAd['ad_id']);

			unset($aAds[$adId]);
		} else {
			OX_Delivery_logMessage('passed _adSelectCheckCriteria: bannerid '.$aAd['ad_id'], 7);
	//		$log->logDebug("passed adSelectCheckCriteria for banner id: ".$aAd['ad_id']);
...
$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
$date=date("Y-m-d");
$Daily_query=mysql_query("select * from oxm_report where date='$date' LIMIT 0 , 1")or die('Error in daily Update');

if(mysql_num_rows($Daily_query)<=0)
{
	mysql_query("update oxm_notification set budmail_status='0'")or die('Error in daily Update1');
...
if (!empty($row['html'])) {
	$cookie = array();
	global $poiIndx;
	$poiIndxCurrentBannerId = $row['bannerid'];
// Modified on 15 oct
//	$cookie[$conf['var']['adIdKey']] = $row['bannerid'];
	...
	for($i = 0; $i < count($poiIndx); $i++)
	{
		...
		if($poiIndx[$i]['adId'] == $row['bannerid'])
		{

			if(!empty($poiIndx[$i]['poiId']) && ($poiIndx[$i]['poiId'] != -1))
			{
				$cookie[$conf['var']['POIKey']] = $poiIndx[$i]['poiId'];
				break;
			}
		}
	}
