...
$poiVal;
$bannerurl;
$landingArray['call']="false";
$landingArray['web']="false";
$landingArray['map']="false";
...
$globDate;
$globalarr;
function IsIntermediateLandingEnable($bannerid)
{

        global $log;
        global $globalarr;	
	//$log->logDebug("\n entered ISIntermediateLandingEnable()  ".$bannerid."  global arr is ".print_r($globalarr,true));
	$result['redirecturl'] = "web";
	$result['resultent'] = "redirect_web";

	$conf = $GLOBALS['_MAX']['CONF'];
	$con2 = mysql_connect($GLOBALS['_MAX']['CONF']['database']['host'],$GLOBALS['_MAX']['CONF']['database']['username'],$GLOBALS['_MAX']['CONF']['database']['password']);
	if(!$con2)
	{
		$log->logError("ck.php fail to connect mysql. \n");
		die('Not connected : ' . mysql_error());
	}
	$dbSelect = mysql_select_db($GLOBALS['_MAX']['CONF']['database']['name'], $con2);

	if(!$dbSelect)
	{
		$log->logError("ck.php fail to select database. \n");
		die('Not connected : ' . mysql_error());
	}
	$_bannerQuery = mysql_query("select * from ox_banners where bannerid=".$bannerid);
	$_bannerValues = mysql_fetch_assoc($_bannerQuery);


	$_campaignQuery = mysql_query("select * from djx_campaign_limitation where campaignid=".$_bannerValues['campaignid']);
	$_campaignValues = mysql_fetch_assoc($_campaignQuery);

	if($_campaignValues['compiledlimitation'] != '')
	{
		if (strpos($_campaignValues['compiledlimitation'],'intermediate_landing') !== false)
		{
			global $landingArray;
			if(strpos($_campaignValues['compiledlimitation'],'call') !== false)
			{
				$landingArray['call']="true";
			}
			if(strpos($_campaignValues['compiledlimitation'],'web') !== false)
			{
				$landingArray['web']="true";
			}
			if(strpos($_campaignValues['compiledlimitation'],'map') !== false)
			{
				$landingArray['map']="true";
			}
			$result['redirecturl'] = "view.php";
			$result['resultent'] = "landing";
			return  $result;
		}
		else if(strpos($_campaignValues['compiledlimitation'],'redirect') !== false)
		{
			global $landingArray;
			if(strpos($_campaignValues['compiledlimitation'],'call') !== false || strpos($_campaignValues['compiledlimitation'],'map') !== false)
			{
				...
				if(mysql_num_rows($_distanceQuery) > 0)
				{
					...
					if(strpos($_campaignValues['compiledlimitation'],'call') !== false)
					{
						$result['redirecturl'] = "tel:".$_tableValues['tel'];
						$result['resultent'] = "redirect_call";
					}
					else
					{
						global $globalarr;
						...
						$result['redirecturl'] = "http://".$conf['webpath']['delivery'].'/map.php?maplat='.$_tableValues['lat']."&maplon=".$_tableValues['lon']."&curlat=".$latitude."&curlon=".$longitude;
						$result['resultent'] = "redirect_map";
					}


				}
			}
			else if(strpos($_campaignValues['compiledlimitation'],'web') !== false)
			{
				$result['redirecturl'] = "web";
				$result['resultent'] = "redirect_web";
		...
		{
			$log->logDebug("No banner has no intermediate landing page. \n");
		}
	}
	else
	{
		$log->logDebug("No targetting creteria found \n");
...
{
	global $log;
	$log->logDebug("\n entered getValueFromKey");
	$val;
	$conf = $GLOBALS['_MAX']['CONF'];
	if(strcmp($conf['var']['POIKey'], $key) == 0)
	{
		$val = getPoiofClient();//$_GET[$conf['var']['POIKey']];
	}
	else if(strcmp($conf['var']['POI'], $key) == 0)
	{
		$val = getPoiofClient();//$_GET[$conf['var']['POI']];
	}
	else
	{
		
		$cookiesStr = $_SERVER['HTTP_COOKIE'];
		$tempArray = explode($key, $cookiesStr);


		$key2 = "%22";
		$tempArray2;

		$tempArray2 = explode($key2,$tempArray[count($tempArray)-1]);
...
function includeDeliveryPluginFile($trackerid, $conversionInfo, $trackerJsCode = null)
{
	$conf = $GLOBALS['_MAX']['CONF'];
	$buffer = '';
...
{
	global $log;
	//$log->logDebug(" \n Entered  MAX_Delivery_log_logAdClick() adid is ".$adId." zoneid is ".$zoneId);
	...
	{
/*		global $poiVal;
		$keyVal	=	$GLOBALS['_MAX']['CONF']['var']['POIKey'];
		$poiVal = 	getValueFromCookies($keyVal);
		$log->logDebug("VAlue of POIKey is :".$poiVal);
*/
		...
		$globCampaignid=$campaignid;
		global $globZoneid;
		$globZoneid = $zoneid;
		global $globBannerid;
		$globBannerid = $bannerid;

		global $globRevenue_type;
		$globRevenue_type= $revenue_type;
		...
		if($revenue_type==$revenue_value)
		{
			...
			if(mysql_num_rows($account_value)>0)
			{
				...
				//Have to done:: Check if location targetting is on
				// and if intermediate landing page is disable then where the click goes (web , map or call)? Update its count.
				...
				if(($accamount>=$perclick))
				{
					if($logclicks)
					{
						...
						// Reporting :: update client account balance and remaing budget of that campaign.
						...
						//get values into global scope and insert count of click to call, web or web.
						// added by neha
						global $globClientid;
						$globClientid = $clientid;
						...
						if($aZone['rtb']==1){
							...
							$query = mysql_query("select a.ad_id, r.campaign_id,cl.clientid,r.id as rowrtbId, r.bid_rate as bidding from ".$table_prefix."ad_zone_assoc as a,".$table_prefix."banners as b,".$table_prefix."campaigns as c,".$table_prefix."clients as cl, aff_rtb as r where a.ad_id=b.bannerid and b.campaignid=c.campaignid and c.clientid = cl.clientid and b.campaignid = r.campaign_id and (c.expire_time>=NOW() or c.expire_time is NULL) and (activate_time is NULL or activate_time<=NOW()) and a.zone_id =".$zoneid." order by r.bid_rate DESC") or die("Error1".mysql_error());
							...
							$detection=mysql_fetch_array(mysql_query("SELECT c.clientid,d.accbalance,a.revenue_type,a.campaignid as campaignid  FROM `oxm_accbalance` as d,ox_banners as b,ox_clients as c,ox_campaigns as a where c.clientid=a.clientid and a.campaignid=b.campaignid and d.clientid=c.clientid and b.bannerid=".$adId));



							$tardet1=mysql_query("select * from aff_rtb where campaign_id in(".$detection['campaignid'].") and bid_rate!='0'")or die("Error2".mysql_error());
							$amount=0;
							while($fetchbidvalue=mysql_fetch_array($tardet1))
							{
								...
								$getcampname=mysql_fetch_array(mysql_query("select * from ox_campaigns where campaignid='".$fetchbidvalue['campaign_id']."'"))or die("Error1".mysql_error());
								...
								if($fetchbidvalue['type']=='country')
								{
									...
									if(in_array($countrycode,$allcountry))
									{
										...
										$exdat=mysql_query("select * from aff_rtb_statistics where date='".date('Y-m-d')."' and bid_win_amount=".$bidrate." and match_campaign='".$compadv."' and banner_id=".$adId." and zone_id=".$zoneId." and win_criteria='".$target."'")or die("Error1".mysql_error());
										...
										if(mysql_num_rows($exdat)>0)
										{
											...
											$amount=$valdet['amount']+$bidrate;


											if($detection['revenue_type']==2)
											{


												mysql_query("UPDATE `aff_rtb_statistics` SET clicks= ".$imp.",amount='$amount' where date='".date('Y-m-d')."' and bid_win_amount=".$bidrate." and match_campaign='".$compadv."' and banner_id=".$adId." and zone_id=".$zoneId." and   win_criteria='".$target."' ")or die("Error1".mysql_error());

											}
											else
											{

												mysql_query("UPDATE `aff_rtb_statistics` SET clicks= ".$imp." where date='".date('Y-m-d')."' and bid_win_amount=".$bidrate." and match_campaign='".$compadv."' and banner_id=".$adId." and zone_id=".$zoneId." and win_criteria='".$target."' ")or die("Error1".mysql_error());
											}
		...
		if($_GET['zoneid']==0)
		{
			...
			$date=gmdate('Y-m-d');
			$banner_qry=mysql_query("SELECT c.campaignid,								  
											b.bannerid,								 
											c.clientid,								  							 
								  			c.revenue_type,
								  			c.revenue								 
									...
									WHERE  
							      	...
							      	b.bannerid=".$adId." ") or die("mysqlerror");
							      
			$ban_row=mysql_fetch_array($banner_qry);
			$clientid=$ban_row['clientid'];
			$campaignid=$ban_row['campaignid'];
			$bannerid=$ban_row['bannerid'];
		
			global $globalarr;
			foreach($_GET as $variable => $value) {
				$globalarr[str_replace('amp;','',$variable)] = $value;
			}
			if(!empty($globalarr['poi']))	
				$poiVal=$globalarr['poi'];
			else
				$poiVal=0;  
		//	$poiVal=$_GET['poi'];
			global $globClientid;
			...
			global $globBannerid;
			$globBannerid = $bannerid;	
			$array_val=array("zoneid"=>0,"clientid"=>$clientid,"campid"=>$campaignid,"bannerid"=>$bannerid,"poiId"=>$poiVal);
		
		}
		else
		{
			$array_val=array("zoneid"=>$zoneid,"clientid"=>$clientid,"campid"=>$campaignid,"bannerid"=>$bannerid,"poiId"=>$poiVal);
		}
		log_Store_Click($array_val);
...
{
	global $log;
	$log->logDebug("\n inside MAX_Delivery_log_setLastAction");
	$aConf = $GLOBALS['_MAX']['CONF'];
...
function MAX_redirect($url)
{
	//added by neha
	global $log;
	$log->logDebug("Entered MAX_redirect() ".$url);
/*	global $bannerurl;
	$bannerurl = $url;
	$keyVal = "bannerid";
	...
	if($_GET['type'] >= 1)
	{
		$banId = getValueFromCookies($keyVal);
	...
	*/
	...
	//$log->logDebug("poival is ".$poiVal."  banid is ".$banId);
	$rslt = IsIntermediateLandingEnable($banId);

	if($rslt['resultent'] == "landing")
	{
	//	include $rslt['redirecturl'];
                $log->logDebug("MAX_redirect() landing enabled");
		include "view.php";
		return;
	}
	else
	{
	        $log->logDebug("MAX_redirect() landing not enabled");
		if($rslt['resultent'] != "redirect_web")
		{
			if($rslt['resultent'] == "redirect_call")
			{
				?>
<html>
<body onload="check()" >
<script>
	function check()
	{
	window.location.href="<?php echo $rslt['redirecturl'];  ?>";	
	}
...
$arr	=	array(
		"zoneid"=>$globZoneid,
		"clientid"=>$globClientid,
		"campid"=>$globCampaignid,
		"bannerid"=>$globBannerid,
		"poiId"=>$poiVal);
//$log->logDebug("call arr is ".print_r($arr,true));
log_click_action($arr, $action);
log_click_action_store($arr, $action);
return;
			}
			else if (!preg_match('/^(?:javascript|data):/i', $rslt['redirecturl']))
			{   
                                global  $globClientid;
				global	$globZoneid;
				global	$globBannerid;
				...
				$arr	=	array(
									...
									"bannerid"=>$globBannerid,
									"poiId"=>$poiVal);
				$log->logDebug("redirect url is ".$rslt['redirecturl']);	
				header('Location: '.$rslt['redirecturl']);
		...
		else
		{
			...
			//increase web
                        $log->logDebug("MAX_redirect() increase web");
 			global  $globClientid;
			global	$globZoneid;
			global	$globBannerid;
			global	$globCampaignid;
...
/**
 * This function is used to store the clicks
...
function log_Store_Click($arr)
{
global $log;
$log->logDebug("entered log_store_click()");
	...
	if(!empty($arr))
	{
		...
		{
			$clk	=	$clk+1;
			mysql_query("update oxm_store_report set clicks='$clk' where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId' AND date(date)='$date'");
		}
		else
		{
			$clk	=	1;
			mysql_query("insert into oxm_store_report(clientid,zoneid,campaignid,bannerid,date,poi_id,clicks)values('$clientid','$zoneid','$campid','$bannerid','$date','$poiId','$clk')") or die(mysql_error());
...
/**
 * This function is used to increase click_to_action in oxm_report table when 
 * landing page is not enable
 * @param unknown $arr
 * @param unknown $action
 */
function log_click_action($arr,$action)
{
	...
	if(!empty($arr))
	{
		...
		$query		=	mysql_query("select * from oxm_report where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId'  AND date(date)='$date'");
		$res		=	mysql_fetch_array($query);
		...
		switch($action)
		{
			case 'CALL':
						...
						$clk		=	$res['click_to_call'];
						if(mysql_num_rows($query)>0)
						{
							$clk	=	$clk+1;
							mysql_query("update oxm_report set click_to_call='$clk' where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId' AND date(date)='$date'");
						}
						else
						{
							$clk	=	1;
							mysql_query("insert into oxm_report(clientid,zoneid,campaignid,bannerid,date,poi_id,click_to_call)values('$clientid','$zoneid','$campid','$bannerid','$date','$poiId','$clk')") or die(mysql_error());
			...
			case  'WEB':
						...
						$clk		=	$res['click_to_web'];
						
						if(mysql_num_rows($query)>0)
						{
							$clk	=	$clk+1;
							mysql_query("update oxm_report set click_to_web='$clk' where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId' AND date(date)='$date'");
						}
						else
						{
							$clk	=	1;
							mysql_query("insert into oxm_report(clientid,zoneid,campaignid,bannerid,date,poi_id,click_to_web)values('$clientid','$zoneid','$campid','$bannerid','$date','$poiId','$clk')") or die(mysql_error());
			...
			case 'MAP':
						...
						$clk		=	$res['click_to_map'];
						
						if(mysql_num_rows($query)>0)
						{
							$clk	=	$clk+1;
							mysql_query("update oxm_report set click_to_map='$clk' where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId' AND date(date)='$date'");
						}
						else
						{
							$clk	=	1;
							mysql_query("insert into oxm_report(clientid,zoneid,campaignid,bannerid,date,poi_id,click_to_map)values('$clientid','$zoneid','$campid','$bannerid','$date','$poiId','$clk')") or die(mysql_error());
...
/**
 * This function is used to increase click_to_action in oxm_store_report table when
 * landing page is not enable
 * @param unknown $arr
 * @param unknown $action
 */

function log_click_action_store($arr, $action)
{
	...
	if(!empty($arr))
	{
		...
		switch($action)
		{
			case 'CALL':
				...
				{
					$clk	=	$clk+1;
					mysql_query("update oxm_store_report set click_to_call='$clk' where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId' AND date(date)='$date'");
				}
				else
				{
					$clk	=	1;
					mysql_query("insert into oxm__store_report(clientid,zoneid,campaignid,bannerid,date,poi_id,click_to_call)values('$clientid','$zoneid','$campid','$bannerid','$date','$poiId','$clk')") or die(mysql_error());
			...
			case  'WEB':
				...
				{
					$clk	=	$clk+1;
					mysql_query("update oxm_store_report set click_to_web='$clk' where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId' AND date(date)='$date'");
				}
				else
				{
					$clk	=	1;
					mysql_query("insert into oxm_store_report(clientid,zoneid,campaignid,bannerid,date,poi_id,click_to_web)values('$clientid','$zoneid','$campid','$bannerid','$date','$poiId','$clk')") or die(mysql_error());
			...
			case 'MAP':
				...
				{
					$clk	=	$clk+1;
					mysql_query("update oxm_store_report set click_to_map='$clk' where clientid='$clientid' AND zoneid='$zoneid' AND bannerid='$bannerid' AND campaignid='$campid' AND poi_id='$poiId' AND date(date)='$date'");
				}
				else
				{
					$clk	=	1;
					mysql_query("insert into oxm_store_report(clientid,zoneid,campaignid,bannerid,date,poi_id,click_to_map)values('$clientid','$zoneid','$campid','$bannerid','$date','$poiId','$clk')") or die(mysql_error());
...
function getPoiofClient()
{       global $log;
	$log->logDebug("\n entered getPoiofClient");
	$conf = $GLOBALS['_MAX']['CONF'];
	...
	$table_prefix = $GLOBALS['_MAX']['CONF']['table']['prefix'];
	global $globClientid;
$_distanceQuery = mysql_query("select * from storefrontdata where (clientid='".$globClientid."')") or die("culnot select:n".mysql_error());
	$result = false;
	if(mysql_num_rows($_distanceQuery) > 0)
	{
		$listofMatchedPoi;
		...
		while ($row = mysql_fetch_array($_distanceQuery, MYSQL_BOTH))
		{
			...
			$radiusVal = 5;
			$_radiusQuery = mysql_query("select * from storefront_radius where (advid='".$globClientid."')") or die("culnot select:n".mysql_error());
		...
		$tempPoiIndx = getShortestDistancePoi($listofMatchedPoi);
		return  $tempPoiIndx;
	}
}
